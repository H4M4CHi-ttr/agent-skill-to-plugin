...
The executable policy is centralized in `scripts/agent_skill_to_plugin/limits.py`. Reports record the tool version, schema version, acquisition time, fixed commit or snapshot SHA-256, source file hashes, generated Plugin tree hash, and ZIP hash. Do not copy limits or transient dependency versions into workflow instructions.
...
Configured limits at version 0.5.0 are 5,000 entries, 100 MiB per member, 512 MiB total expanded data, 100 MiB downloaded or compressed data, 20 path levels, and 1,024 path characters. The generated ZIP has one top-level Plugin directory and deterministic member metadata.
